...
def find_in_data(filename: str) -> Path:
    """Trouve un fichier n'importe où sous /data (raw/processed/...)"""
    hits = list(DATA.rglob(filename))
    if not hits:
        raise FileNotFoundError(f"Fichier introuvable sous {DATA}: {filename}")
    # si plusieurs hits, on prend le premier (tu peux trier si besoin)
    return hits[0]
...
def norm_round_kaggle(stage: object) -> str:
    k = norm_txt(stage)
    return ROUND_MAP_KAGGLE.get(k, "" if stage is None else str(stage))
...
# ---------------------------
def main() -> None:
    # Auto-find Kaggle files (corrige tes erreurs d'emplacement)
    KAGGLE_MATCHES = find_in_data("matches.json")
    KAGGLE_TOURNAMENT = find_in_data("tournament.json")
    ...
    print("[INFO] V1:", V1_FILE)
    print("[INFO] Kaggle matches:", KAGGLE_MATCHES)
    print("[INFO] Kaggle tournament:", KAGGLE_TOURNAMENT)

    # Load V1
    df = pd.read_csv(V1_FILE)

    # ⚠️ FIX MERGE: edition doit être numérique des deux côtés
    # (sinon: "merge on object and int64 columns for key 'edition'")
    ...
    # Types + keys Kaggle
    df_k["edition"] = pd.to_numeric(df_k["edition"], errors="coerce").astype("Int64")
    ...
    # ---------------------------
    # PASS 2 : edition + teams + city + round (fallback)
    # ---------------------------
    ...
    drop_cols = [c for c in df1.columns if c in ["round_kaggle", "city_kaggle", "date_kaggle"]]
    drop_cols += [c for c in df1.columns if c.endswith("_kaggle")]
    drop_cols += [c for c in ["home_key", "away_key", "round_key", "city_key"] if c in df1.columns]
    ...
    df_out.to_csv(OUT_V2, index=False, encoding="utf-8")
    print(f"OK -> {OUT_V2}")
